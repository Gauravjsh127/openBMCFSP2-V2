...
EOF
  ...
  RUN cd git*; make prefix=/usr/local/git all; make prefix=/usr/local/git install;echo "export PATH=/usr/local/git/bin:$PATH" >> /etc/bashrc;source /etc/bashrc
  RUN git --version
...
cd tmp/deploy/images/fsp2-x86
mkdir rootfs
cp core-image-minimal-x86-fsp2-x86.cpio.gz rootfs/
cd rootfs
gzip -cd core-image-minimal-x86-fsp2-x86.cpio.gz | cpio -idmv
...
## copying crash tools build inside docker image to fsp2-x86 rootfs folder
cp ../crash-ppc32-cross/crash tmp/deploy/images/fsp2-x86/rootfs/usr/bin/
cp ../crash-ppc32-cross/extensions/*.so tmp/deploy/images/fsp2-x86/rootfs/usr/lib/crash/extensions/
